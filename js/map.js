...
(() => {
  ...
  const state = { places: [], coordinates: [], details: {}, filtered: [], pinned: null, projection: null, cityCenters: new Map(), cityNames: [] };
  ...
  function cityForPlace(place) {
    if (place?.prefecture_city) return place.prefecture_city;
    const text = `${place?.modern_name || ''} ${place?.historical_name || ''}`;
    return state.cityNames.find(city => text.startsWith(city.replace(/市$/, ''))) || state.cityNames.find(city => text.includes(city.replace(/市$/, ''))) || '';
  }
  ...
  function detailCard(placeIds, pinned = false) {
    ...
    placeIds.forEach((placeId, index) => {
      ...
      if (coord.status !== 'located') section.append(el('p', city ? `地图展示：${city}市域代表点（非历史遗址坐标）` : '地图展示：无法确定所属设区市，未生成代表点', 'pending-map-note'));
  ...
  function renderBoundary(boundary) {
    state.projection = makeProjection(boundary); state.cityNames = boundary.features.map(feature => feature.properties.city_name); state.cityCenters.clear(); const layer = byId('boundary-layer'), labels = byId('city-label-layer'); layer.replaceChildren(); labels.replaceChildren();
    boundary.features.forEach(feature => { const region = feature.properties.region, city = feature.properties.city_name, path = svgEl('path'); path.setAttribute('d', pathData(feature.geometry, state.projection)); path.setAttribute('class', `city-boundary region-${region === '苏南' ? 'south' : region === '苏中' ? 'central' : 'north'}`); path.dataset.city = city; layer.append(path); const [x, y] = visualCenter(feature.geometry); state.cityCenters.set(city, [x, y + 21]); const label = svgEl('text'); label.textContent = city; label.setAttribute('x', x.toFixed(1)); label.setAttribute('y', y.toFixed(1)); label.setAttribute('class', 'city-label'); label.dataset.city = city; labels.append(label); }); byId('map-status').hidden = true;
  }
  function mapPoint(coord) { const place = state.places.find(item => item.place_id === coord.place_id); if (coord.status === 'located') return state.projection([coord.longitude, coord.latitude]); return state.cityCenters.get(cityForPlace(place)) || null; }
  function projectedGroups(items) { const candidates = items.map(item => ({ ...item, point: mapPoint(item) })).filter(item => item.point), groups = []; candidates.forEach(item => { const group = groups.find(existing => existing.some(member => Math.hypot(member.point[0] - item.point[0], member.point[1] - item.point[1]) <= GROUP_DISTANCE && member.status === item.status)); if (group) group.push(item); else groups.push([item]); }); return groups; }
  function renderMarkers() {
    const layer = byId('marker-layer'); layer.replaceChildren(); if (!state.projection) return;
    projectedGroups(state.filtered).forEach(items => { const pending = items.every(item => item.status !== 'located'), placeIds = [...new Set(items.map(item => item.place_id))], x = items.reduce((sum, item) => sum + item.point[0], 0) / items.length, y = items.reduce((sum, item) => sum + item.point[1], 0) / items.length, group = svgEl('g'); group.setAttribute('class', `map-marker${pending ? ' marker-pending' : ''}`); group.setAttribute('tabindex', '0'); group.setAttribute('role', 'button'); group.setAttribute('transform', `translate(${x} ${y})`); group.dataset.placeIds = placeIds.join(','); const names = placeIds.map(id => { const place = state.places.find(candidate => candidate.place_id === id); return place?.historical_name || place?.modern_name; }).filter(Boolean).join('、'); group.setAttribute('aria-label', `查看${names}的家训信息${pending ? '，位置待核，地图为市域代表点' : ''}${placeIds.length > 1 ? `，共${placeIds.length}个地点` : ''}`); const halo = svgEl('circle'); halo.setAttribute('r', '16'); halo.setAttribute('class', 'marker-halo'); group.append(halo); if (pending) { const symbol = svgEl('polygon'); symbol.setAttribute('points', '0,-8 8,0 0,8 -8,0'); symbol.setAttribute('class', 'marker-pending-symbol'); group.append(symbol); } else { const dot = svgEl('circle'); dot.setAttribute('r', '7'); dot.setAttribute('class', 'marker-dot'); group.append(dot); } if (placeIds.length > 1) { const count = svgEl('text'); count.textContent = String(placeIds.length); count.setAttribute('class', 'marker-count'); count.setAttribute('x', '12'); count.setAttribute('y', '-10'); group.append(count); } group.addEventListener('mouseenter', () => { if (!state.pinned) updateSidePanel(placeIds); }); group.addEventListener('focus', () => { if (!state.pinned) updateSidePanel(placeIds); }); group.addEventListener('click', event => { event.stopPropagation(); state.pinned = placeIds; updateSidePanel(placeIds, true); }); group.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); state.pinned = placeIds; updateSidePanel(placeIds, true); } if (event.key === 'Escape') clearPinned(); }); layer.append(group); });
  }
  function placeMatches(coord, filters) { const place = state.places.find(item => item.place_id === coord.place_id), records = (place?.records || []).map(item => state.details[item.record_id]).filter(Boolean); return (!filters.city || cityForPlace(place) === filters.city) && (!filters.precision || coord.precision === filters.precision) && (!filters.era || records.some(record => record.era === filters.era)) && (!filters.level || records.some(record => record.verification_level === filters.level)); }
